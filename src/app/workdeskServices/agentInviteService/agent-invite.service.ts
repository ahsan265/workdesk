...
export class AgentInviteService {
  ...
  constructor(private GigaaaApiService: GigaaaApiService,
    private CommonEndpoints: CommonEndpoints, private router: ActivatedRoute) { }

  public async sendtAgentInvitationCode() {
    let code = this.getInvitationCode();
    if (code != null) {
      let code_invite = { "invitation_code": JSON.parse(code) }
      await this.GigaaaApiService.sendinvitationcode(this.CommonEndpoints.getEpsParamLocal().token, code_invite).finally(() => {
  ...
  public getInvitedAgentDetails() {
    this.router.queryParams.subscribe(data => {
      ...
      if (code != null) {
        ...
        this.GigaaaApiService.getinvitationdetails(code).subscribe((data: any) => {
          console.log(data)
          if (data != null) {
