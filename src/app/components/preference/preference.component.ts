...
export class PreferenceComponent {
  ...
  selectedWidgetOutput(event: tickerModel) {
    switch (event.name) {
      case 'call':
        this.callWaitTime = this.CommonService.toSeconds(0, event.tickers[0].value, event.tickers[1].value)
        break;
      case 'user':
        this.userWaitTime = this.CommonService.toSeconds(0, event.tickers[0].value, event.tickers[1].value)
        break;
      case 'chat':
        this.chatWaitTime = this.CommonService.toSeconds(event.tickers[0].value, event.tickers[1].value, event.tickers[2].value)
        break;
    }
  }

  saveData() {
    this.PreferenceService.updateDefaultTimes(this.callWaitTime, this.userWaitTime, this.chatWaitTime)
  }
