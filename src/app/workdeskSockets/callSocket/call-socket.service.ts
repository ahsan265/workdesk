...
export class CallSocketService {
  ...
  // dial webrtcCall
  public async dialCall(calluuid: string, userid: string, is_refresh: boolean, browser_name: string, connectionid: string): Promise<any> {
    let url = this.websocket_url + "/webrtc?call_uuid=" + calluuid + "&peer_id=" + userid + "&is_refreshed=" + is_refresh + "&browser_name=" + browser_name + "&connection=" + connectionid;
  ...
  private async recalSocket(): Promise<any> {
    ...
    if (callstat != null) {
      ...
      await this.dialCall(callid.call_uuid, callstat?.user_id, callstat?.is_refreshed, this.DevicesInformationService.getBrowserName(), connectionId?.connection);
    }
  }
