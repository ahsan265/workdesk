...
import { agentOperationInformationModel, CallControlModel, inputOuputdevices } from 'src/app/models/callInterfaceModel';
...
import { inputDevices, maximizeCallControlData, minimizeCallControlData, outputDevice, videoMinimizeControlData } from '../callsInterfaceData';
...
export class CallControlsComponent implements OnInit {
  ...
  onOffMicrophone(event: boolean) {
    (event === false) ? this.StreamingService.muteAudio() : this.StreamingService.unmunteAudio();
  ...
  openDeviceSwitcherPopup(event: boolean) {
    ...
    (event) ? this.openDeviceSwitcher = true : this.openDeviceSwitcher = false;
  ...
  selectInputOutputDeviceInformation(event: inputOuputdevices) {
    if (event.deviceType === "audioinput") {
      this.inputDeviceData.devices.forEach(data => {
        (data.id === event.id) ? data.isSelected = true : data.isSelected = false;
      })
    }
