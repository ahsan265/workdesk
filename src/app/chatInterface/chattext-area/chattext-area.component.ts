...
export class ChattextAreaComponent implements OnDestroy {
  ...
  constructor(
    ...
    this.ChatSocketService.liveChatThread.asObservable().subscribe((data) => {
      data.selected_thread !== '' && data.data.length !== 0
    ...
    this.ChatSocketService.typingMessage.asObservable().subscribe((data) => {
      this.selectedChatThreadModelData =
        this.ChatSocketService.lastSelectThreadUuid.getValue();
      this.selectedChatThreadModelData.uuid === data.data.conversation_uuid &&
      data.isTyping
  ...
  sendMessage() {
    if (
      ...
      this.ChatSocketService.sendMessageDataText(
        this.messageForm.controls?.message.value,
        this.ChatSocketService.lastSelectThreadUuid.getValue()
      );
  ...
  checkReadMessage() {
    this.ChatOperationService.getAllUnreadMessage().length !== 0
      ? this.ChatOperationService.setUnreadToRead(
          this.ChatSocketService.lastSelectThreadUuid.getValue().uuid,
  ...
  typingEvent() {
    if (this.ChatSocketService.lastSelectThreadUuid.getValue().uuid !== '') {
      this.ChatSocketService.sendIsTypingMessage(
        this.ChatSocketService.lastSelectThreadUuid.getValue().uuid
      );
    }
  }
