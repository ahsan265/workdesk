...
import { IncomingCallModel, IncomingCallModelTable } from 'src/app/models/callModel';
...
export class IncomingComponent {
  ...
  constructor(private callservice: CallsService, private CommonService: CommonService,
    ...
    this.callservice.sendDataToIncomingTabsSubject.subscribe((data: IncomingCallModel[]) => {
      this.incomingData = data.map((incomingData) => ({
        hashIcon: '#',
        call_uuid: incomingData.call_uuid,
        language_icon: '',
        utilites: [{ image: this.CommonService.getLanguageFlags(incomingData.language_id) },
        { image: this.CommonService.getBrowserFlag(incomingData.browser) }, {
          image: this.CommonService.getDeviceType(incomingData.desktop)
        }, { image: this.CommonService.getOperatingSystem(incomingData.operating_system) }],
        callType: { image: this.CommonService.getConversationType(incomingData.is_video), text: this.callservice.getCallType(incomingData.is_video) },
        name: incomingData.name,
        user_id: this.callservice.getUserId(incomingData.user_id),
        time: this.callservice.getElapsedTime(incomingData.waiting_started_at),
  ...
  async getCallsId(event: string) {
    let data = { "call_uuid": event }
     await this.GigaaaApiService.getcalltype(this.CommonService.getEndpointsParamLocal().token,
