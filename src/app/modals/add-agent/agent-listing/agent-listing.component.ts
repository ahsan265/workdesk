...
import { InviteAgentModel, agentInvitationList } from 'src/app/models/agent';
...
export class AgentListingComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.AgentSocketService.freeSeatsInformation
      .asObservable()
      .subscribe((data) => {
  ...
  async addAgentListing() {
    if (this.agentIvitationList.length !== this.freeSeatCount) {
      if (this.isValidEmail === true && this.langaugeIds.length !== 0) {
        this.agentIvitationList.push({
          email: this.email,
          language: this.langaugeIds
        });
  ...
  async sendAgentInformation() {
    ...
    if (isAllDataOk) {
      const agentListedData: InviteAgentModel[] = this.agentIvitationList.map(
        (data) => ({
          email: data.email,
          role: 'agent',
      ...
      this.MessageService.setSuccessMessage(
        'Agent invitation(s) has been sent.'
      );
  ...
  getEmailByIndex() {
    if (
      /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(
        this.emailTyped.get(0)?.nativeElement.value
      )
    ) {
